...
describe('SignUp Controller', () => {
  ...
  it('should return 400 if passwordConfimation fails', async () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
        name: 'any_name',
  ...
  it('should call EmailValidator with correct requested email', async () => {
    const { sut, emailValidatorStub } = makeSut()
  ...
  it('should return 500 if an AddAccount throws', async () => {
    ...
    jest.spyOn(addAccountStub, 'add').mockRejectedValueOnce(() => {
      throw new Error()
    })
    const httpRequest = {
      body: {
